...
class Hex {
    ...
    toString(): string {
        ...
        return `${this.systemName} ${('0000'+this.hexNumber).slice(-4)} ${this.world.uwp} ${this.basesToString()} ${this.tradeClassificationsToString()} ${this.travelZone ? this.travelZone : '-'} ${this.gasGiant ? 'G': '-'}`;
        } else {
    ...
    tradeClassificationsToString(): string {
        if (this.world) {
            return this.world.tradeClassifications.join(", ");
        } else {
            return "";
        }
    }
